...
Kmères utilisés                 | `r colnames(N50)`
...
colorData <- c()
x <- x.kmère[select.kmère]
N50.select <- as.numeric(dataQuality$N50[select.kmère])
colorData <- data.frame(x)
plotN50 <- ggplot(dataQuality, aes(x=x.kmère,y=N50))+geom_point(col='steelBlue')
...
L50.select <- dataQuality$L50[select.kmère]
plotL50 <- ggplot(dataQuality, aes(x=x.kmère,y=L50))+geom_point(col='steelBlue')
...
E.size.select <- dataQuality$E.size[select.kmère]
plotE.size <- ggplot(dataQuality, aes(x=x.kmère,y=E.size))+geom_point(col='steelBlue')
...
max.select <- dataQuality$max[select.kmère]
plotmax <- ggplot(dataQuality, aes(x=x.kmère,y=max))+geom_point(col='steelBlue')
...
n.500.select <- dataQuality$n.500[select.kmère]
plotn.500 <- ggplot(dataQuality, aes(x=x.kmère,y=n.500))+geom_point(col='steelBlue')
...
write.table(selection, file = paste(pathDir,'data_report/select.csv',sep=''), append = FALSE, quote = FALSE, sep = "\t", na = "-",col.names = FALSE)
