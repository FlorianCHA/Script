			...
			if line[0] != "#":
				...
				if tabLine[1] == "AUGUSTUS":
					...
					if typeSeq == "gene":
						...
						outFileGff.write(newline.replace("AUGUSTUS","braker_BGPI"))
					...
					# First CDS add polypeptide start ID=g1.t1.CDS1;Parent=g1.t1
						...
						startPoly = tabLine[3]
						stopPoly = tabLine[4]
						CDSName = tabLine[8].split(";")[0].replace("g"+geneNumero,"Mo_"+strainName+"_braker_"+geneNumeroReformat).replace("ID=","")
					...
					# exon ID=g1.t1.exon2;Parent=g1.t1;
						...
						exonName = tabLine[8].split(";")[0].replace("g"+geneNumero,"Mo_"+strainName+"_braker_"+geneNumeroReformat).replace("ID=","")
					...
					# intron start_codon stop_codon
						...
						Name = tabLine[8].split(";")[0].replace("g"+geneNumero,"Mo_"+strainName+"_braker_"+geneNumeroReformat).replace("Parent=","")
						Line = "%s\tParent=%s\n" % ("\t".join(tabLine[:8]), mRNAName)
				...
				outFile.write(line.replace("AUGUSTUS","braker_BGPI"))
