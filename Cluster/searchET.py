...
########### Gestion directory ##############
    ...
    print(
        ...
        "            Welcome in searchET (Version " + version + ")         ",
    ...
    with open(output,'w') as output_file,open(output.replace('.','_value.'),'w') as output_file_script :
        for elt in dico_id.keys() :
            ...
            if os.path.exists(ET_path) :
                ...
                print(form('-' * 100, 'white', 'bold'))
                print(form(f"Traitement de l'isolat {elt}", 'green', 'bold'))
                print(form(	f"Warning : Il n'y a pas de fichier d'annotation des éléments transposable de l'isolat {elt} dans le repertoire {directory.split('/')[-1]}\n",'orange', 'bold'))
                        ...
                        if type == 'mRNA' and id in dico_id[elt].keys() :
                            ...
                            if Scaffold in dico_ET.keys() :
                                ...
                                for ET in listeET :
                                    ...
                                    stop_ET = int(stop_ET)
                                    id_ET = id_ET.split('Motif:')[1].split('"')[0]
                                    ##### Processing of sequence brin + #####
                                    if brin == '+' :
                                        ##### Search ET in the promoteur #####
                                        if start > start_ET > (start - 400):
                                            if len(re.findall('\([A-Z]*\)n', id_ET)) > 0 : # Test if the ID of the element transposable have the (GTA)n pattern which correspond to micro satellite.
                                                output_file.write(
                                                    f"\t - {id} have the {id_ET} micro-satellite in the 400 nucleotides before the sequence\n")
                                                output_file_script.write(f"{id}\t2\n")
                                            else :
                                                output_file.write(
                                                        f"\t - {id} have the {id_ET} element transposable in the 400 nucleotides before the sequence\n")
                                                output_file_script.write(f"{id}\t1\n")

                                        elif start > stop_ET> (start - 400) :
                                            output_file.write(f"\t - {id} have a part of the  {id_ET} element transposable or Micro-satellite in the 400 nucleotides before the sequence\n")
                                            output_file_script.write(f"{id}\t5\n")

                                        ##### Search ET in the sequence #####
                                        elif start < stop_ET < stop: # If the end of the element transposable is in sequence, the ET is in sequence
                                            if len(re.findall('\([A-Z]*\)n', id_ET)) > 0 :
                                                output_file.write(
                                                    f"\t - {id} have the {id_ET} micro-satellite in the sequence\n")
                                                output_file_script.write(f"{id}\t4\n")
                                            else :
                                                output_file.write(
                                                        f"\t - {id} have the {id_ET} element transposable in the sequence\n")
                                                output_file_script.write(f"{id}\t3\n")

                                        elif start < start_ET < stop and stop_ET > stop :
                                            output_file.write(
                                                f"\t - {id} have a part of the  {id_ET} element transposable or Micro-satellite in the sequence\n")
                                            output_file_script.write(f"{id}\t5\n")
                                    ...
                                    ##### Processing of sequence brin - #####
                                            ...
                                            if stop < stop_ET < (stop + 400) :
                                                if len(re.findall('\([A-Z]*\)n',id_ET)) > 0:  # Test if the ID of the element transposable have th (GTA)n pattern which correspond to micro satellite.
                                                    output_file.write(
                                                        f"\t - {id} have the {id_ET} micro-satellite in the 400 nucleotides before the sequence\n")
                                                    output_file_script.write(f"{id}\t2\n")
                                                else:
                                                ...
                                                output_file.write(
                                                    f"\t - {id} have a part of the  {id_ET} element transposable or Micro-satellite in the 400 nucleotides before the sequence\n")
                                                output_file_script.write(f"{id}\t5\n")

                                            ##### Search ET in the sequence #####
                                            elif start < start_ET < stop : # If the end of the element transposable is in sequence, the ET is in sequence
                                                if len(re.findall('\([A-Z]*\)n', id_ET)) > 0:
                                                    output_file.write(
                                                        f"\t - {id} have the {id_ET} micro-satellite in the sequence\n")
                                                    output_file_script.write(f"{id}\t4\n")
                                            ...
                                            elif start < stop_ET < stop and start_ET < start :
                                                output_file.write(
                                                    f"\t - {id} have a part of the  {id_ET} element transposable or Micro-satellite in the sequence\n")
                                                output_file_script.write(f"{id}\t5\n")
                                ...
                                if start < 400  and brin == '+':
                                    ...
                                    output_file_script.write(f"{id}\t3\n")
                                elif str(dicofasta_scaffold[Scaffold].seq)[start:stop].count('N') > 20 :
                                    output_file.write(f"\t - {id} have the {str(dicofasta_scaffold[Scaffold].seq)[start:stop].count('N')} N in the sequence\n")
                                    output_file_script.write(f"{id}\t8\n")
