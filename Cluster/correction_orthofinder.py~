...
output = '/homedir/gladieux/work/magMax_project/4_Orthologie/3_correction/'
...
for line in lines :
	...
	if len(dico_OG[OG]) != len(listeProtein):
		...
		os.system('tblastn -query %s -db %s -evalue 1e-4 > %s'%(fasta_output,pathDBGenome,blast_output))	
		f = open(blast_output)
		lines1 = f.readlines()
		f.close()
		pathResult = tmp+OG+'_gene.txt'
...
lines = f.readlines()
f.close()
result = open(ouput+'Orthogroups_correction.csv','w')
for line in lines :
	OG = line.split('\t')[0]
...
f = open(ouput+'Orthogroups.txt','r')
lines = f.readlines()
f.close()
...
lines = f.readlines()
f.close()
result = open(ouput+'Orthogroups.GeneCount_correction.txt','w')
for line in lines :
	OG = line.split('\t')[0]
	lineSplit = line.split('\t')
