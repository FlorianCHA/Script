    ...
    if DP < DPmin or MQ < MQmin:
        ...
        if QD >= QDmin:
            if diploidy :
                ...
                elif infoDiploidy == '0/1' or infoDiploidy == '1/0' :
                    if len(ref) == 1 and len(alt) == 1:
                        ac = IUPAC[ref+alt]
                    elif len(ref) >= len(alt) :
                        ac = alt + 'N'*(len(ref) -1)
...
if __name__ == "__main__":
    ...
    if gff != 'None':
        with open(vcf, "r") as f:
            for line in f :
                if line[0] != '#' :
                    vcf_scaffold, position, _, ref, alt, qual, _, info, *_, = line.split()
                    if 'MQ=' not in line and 'DP=' not in line:
                        ac = 'N'
                    else:
                        ...
                        if vcf_scaffold in dicoRNA.keys() and position in dicoRNA[vcf_scaffold].keys() :
                            if position in listePosition :
                                ...
                                if len(ref) > len(ac) :
                                    ac_new = recupAC(id, line, MQmin, DPmin, QDmin)
    ...
    if gff !='None' :
        ...
        with open('{0}_gene.fasta'.format(prefix),'w') as output_gene :
            ...
            for id in sorted(dicoSeq.keys() ,key = sort_human):
                seq = dicoSeq[id]
                record = SeqRecord(Seq(seq), id=id, name=id, description=  f'| length : {len(sequence)}')
                SeqIO.write(record, output_gene, "fasta")
