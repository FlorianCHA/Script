...
# Import module_Flo
from module_Flo import verifDir, createDir, form, isFasta, recupId, verifFichier, fasta2dict, sort_human,indexEgale,comparaisonListe
...
def filtrAln(file):
    ...
    for elt in dico.keys() :
        seqRef = str(dico[elt].seq)
        positionC = indexEgale(seqRef,'C')
        if positionC != old_positionC :
            NewpositionC = comparaisonListe(positionC,old_positionC)
            if len(NewpositionC) == 2 :
                positionC = NewpositionC
                break
        old_positionC = positionC
    print(positionC)
    listeGene =  []
    nb = 0
    ...
    with open(file,'w') as f ,open(file.replace('_filtred_no_redundant.fasta','_bad.fasta'),'w') as f_bad:
...
if __name__ == "__main__":
    ...
    # nbC = filtrAln('%s_filtred.fasta'%newAlignement)
    ...
    while True:
        ...
        os.system('hmmbuild --amino %s %s_filtred_no_redundant.fasta > trash' % (profil, newAlignement))
        f = open(newAlignement + '_filtred.fasta', 'r')
