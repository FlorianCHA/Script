...
########### Gestion directory ##############
	...
	name = fastaFile.split('/')[-1].replace('.gff3','')
			...
			if scaffoldA == scaffoldB :
				...
				if typeB == 'gene' :
					...
					if (startB <= endA <= endB) or (startB <= startA <= endB) or (startA <= endB <= endA) or (startA <= startB <= endA): 
						...
						liste1.remove(elt)
						break
	numberGeneBraker = brakerlines[-1].split('=g')[-1].split('.t1')[0]
	print('Comparaison done')
	os.system('renameGFF.py -g %s%s -s %s -o %s%s_braker.gff3'%(BrakerPath,File,name,outDir,name))
